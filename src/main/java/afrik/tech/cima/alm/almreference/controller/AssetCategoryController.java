package afrik.tech.cima.alm.almreference.controller;

import afrik.tech.cima.alm.almreference.service.AssetCategoryService;
import afrik.tech.cima.alm.common.controller.AbstractCrudController;
import afrik.tech.cima.alm.common.dto.ReferenceDto;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/api/v1/referentiels-alm/categories-actifs")
public class AssetCategoryController extends AbstractCrudController<ReferenceDto> {
    public AssetCategoryController(AssetCategoryService service) {
        super(service);
    }
}
